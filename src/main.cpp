...
#if defined(NDEBUG)
# error "Tatsumicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x78a43e5fec2d70f92c7724a32edd93a893c6689a173905bd15a25e5ff8672e47");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tatsumicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tatsumicoin Signed Message:\n";
...
{
    // Tatsumicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Tatsumicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tatsumicoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Tatsumicoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tatsumicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tatsumicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbbf1dca5fd7fbc330f9561d571800cceaae99cd8493847df5362ebc35b2949fb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "London Times 2017-2018 I loved anime";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041e505267ad0cc5ef65ddc90b93bae79853bd9a42103c061efa4ec0a4e19047527067b493fe91fc47344f48eeb5bc998ce530d46e68d1b053ac129f955f05dcd2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534570539;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1359495;

        if (fTestNet)
        {
            block.nTime    = 1534570537;
            block.nNonce   = 1450268;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7a8a4101b2c7b3e5be0c62308b248ce84b8655ea2239f0518aa58c828b2b9b6c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd4 }; // Tatsumicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TatsumicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TatsumicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TatsumicoinMiner(CWallet *pwallet)
{
    printf("TatsumicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tatsumicoin-miner");
    ...
    try { loop {
        ...
        printf("Running TatsumicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TatsumicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TatsumicoinMiner, pwallet));
}
